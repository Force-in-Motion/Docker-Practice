...
# TODO 'ls' - Показывает содержимое текущей директории


# TODO 'cat' - Показывает содержимое файла в текущей директории, например 'cat main.py'
...
#  (ОПЦИОНАЛЬНО) если для функционирования приложения в контейнере требуется установка дополнительных пакетов / библиотек, то в Dockerfile добавляется инструкция RUN, например: 'RUN pip install pymongo', все установки пакетов подобным образом добавляются после инструкции 'WORKDIR /app'
...
# TODO 'docker compose up' - Запускает создание контейнеров на основе файла конфигурации docker-compose.yml


# TODO 'docker compose up -d' - Запускает создание контейнеров на основе файла конфигурации docker-compose.yml  в фоновом процессе


# TODO 'docker compose down' - Останавливает все активные контейнеры созданные контейнеров на основе файла конфигурации docker-compose.yml и удаляет их


# Пере сборка docker compose после изменений в, запущенном в контейнере, проекте
# TODO 'docker compose up -d --build' - Запускает создание контейнеров на основе файла конфигурации docker-compose.yml в фоновом процессе , а флаг --build пере собирает образы и контейнеры созданные по этим образам,
#  это необходимо делать каждый раз, после того как, были внесены изменения в проект, запущенный в контейнере